...
// Composição editorial: retrato P&B dissolvendo no fundo + dois cards coloridos flutuando
function FounderShot() {
  ...
  return (
    <div ref={ref} className={`founder-shot${inView ? " in" : ""}`}>
      {/* moldura deslocada atrás */}
      <div className="founder-frame" />

      {/* retrato principal */}
      <div className="founder-main">
        <img src="/founder-main.jpg" alt="Sostenes, co-fundador e mentor do Instituto Hands On" loading="lazy" />
        <div className="founder-tint" />
        <div className="founder-fade" />
        <div className="founder-caption">
          <span style={{ color: "var(--text-3)" }}>{"// "}</span>fundador &amp; mentor
          <strong>Sostenes</strong>
        </div>
      </div>

      {/* cards coloridos */}
      <figure className="founder-card founder-card-a">
        <img src="/founder-laptop.jpg" alt="Sostenes trabalhando no notebook" loading="lazy" />
        <figcaption><span className="dot" style={{ background: "var(--cyan)" }} />Aulas ao vivo</figcaption>
      </figure>
      <figure className="founder-card founder-card-b">
        <img src="/founder-cup.jpg" alt="Sostenes em pé segurando uma xícara" loading="lazy" />
        <figcaption><span className="dot" style={{ background: "var(--pink)" }} />Mentoria 1:1</figcaption>
      </figure>
    </div>
  );
}
...
export default function Founders() {
  ...
  return (
    <section id="founders" style={{ padding: "5rem 0", background: "var(--surface)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", position: "relative", zIndex: 2, overflow: "hidden" }}>
        ...
        <div className="founders-grid">
          ...
          <div>
            ...
            <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "clamp(34px, 5vw, 58px)", fontWeight: 800, marginTop: "0.75rem", marginBottom: "2rem", letterSpacing: "-0.01em", lineHeight: 1.05 }}>
              Sostenes e Daniel
              <span style={{ color: "var(--cyan-light)", display: "block" }}>a história real do Instituto</span>
            </h2>

            {/* Timeline */}
            <div style={{ display: "flex", flexDirection: "column" }}>
              {steps.map(({ color, label, text }, i) => (
                <div key={i} style={{ display: "flex", gap: "1.25rem", paddingBottom: i < steps.length - 1 ? "1.75rem" : 0 }}>
        ...
        <div style={{ marginTop: "3rem" }}>
          ...
          <div className="stats-mobile">
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: "1rem" }}>
              {stats.map((s, i) => (
                <button key={i} aria-label={s.l} onClick={() => setActive(i)} style={{
