          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "0.75rem", marginTop: "2.5rem" }}>
            {[
              ...
              <div key={l} style={{ padding: "1.25rem 1rem", background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, textAlign: "center" }}>
                <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "clamp(28px, 5vw, 36px)", fontWeight: 800, color: "var(--text-1)", marginBottom: 4 }}>{n}</div>
                <div style={{ fontSize: 13, color: "var(--text-3)", lineHeight: 1.4 }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
